...
Naming Conventions
  ...
  Manifest filename:    104a.mfdb.bejson (fixed — never vary this)
...
Federation Protocols (v1.3 Optional Standard)

MFDB v1.3 introduces optional standards for Master/Slave federation, allowing 
distributed databases to identify their roles and manage connectivity.

1. Network_Role Header (104a Manifest):
   - A custom PascalCase header in the manifest file.
   - Values: "Master" (authoritative source) or "Slave" (downstream replica).

2. ConnectedSlave Entity (Global/Master Registry):
   - Conventionally stored in a Master's global database to track the network.
   - Entity Name: "ConnectedSlave"
   - Recommended Fields:
     - label (string, PK): Human name for the node.
     - url (string): Public endpoint or IP of the node.
     - role (string): "Master" or "Slave" (usually "Slave").
     - status (string): "Active", "Offline", "Syncing".
     - supported_entities (array): List of entity_names this node tracks.
